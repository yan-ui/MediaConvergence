 ...
 * @see #onDestroy
 * @use extends BaseViewHolder<T>, 具体参考.DemoView
 */
public abstract class BaseViewHolder<T> extends RecyclerView.ViewHolder {
	...
	public interface OnViewClickListener {
		 ...
		 * @param v
		 */
		void onViewClick(@NonNull BaseViewHolder bv, @NonNull View v); //BaseViewHolder<T>泛型太麻烦，不利于多层View传递
	}
	 ...
	 * @param listener
	 */
	public BaseViewHolder<T> setOnDataChangedListener(OnDataChangedListener listener) {
	 ...
	 * @param context
	 * @param layoutResId
	 * @see #BaseViewHolder(Activity, int, ViewGroup)
	 */
	public BaseViewHolder(Activity context, @LayoutRes int layoutResId) {
	 ...
	 */
	public BaseViewHolder(Activity context, @LayoutRes int layoutResId, ViewGroup parent) {
	 ...
	 * @param itemView
	 */
	public BaseViewHolder(Activity context, View itemView) {
		super(itemView);
	...
	public void bindView(T data_) {
		if (data_ == null) {
			LogUtils.w("bindView data_ == null");
		}
		this.data = data_;
	...
	public void onDestroy() {
		if (itemView != null) {
			try {
				...
				LogUtils.w("onDestroy  try { itemView.destroyDrawingCache();" +
						" >> } catch (Exception e) {\n" + e.getMessage());
			}
		}
